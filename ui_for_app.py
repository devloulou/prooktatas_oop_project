# csinálunk  fő frame-t - main frame
# létrehozunk egy canvas
# convashoz fogjuk hozzáadni a scrollbart
# a canvast fogjuk beállítani
# létrehozunk egy másodlagos framet-, amit a canvasra fogunk rá illeszteni
...
meta_data_path = r"C:\WORK\Prooktatás\projekt_ui\prooktatas_oop_project\meta_data"
...
class MovieMetaViewer(tk.Tk):
    def __init__(self, image_data, *args, **kwargs):
        ...
        #amely a canvas-ra ráilleszti a képekkel megjelenített frame-t
...
class ImageListFrame(ttk.Frame):
    ...
    def images_to_grid(self):
        ...
        for _idx, item in enumerate(self.image_data):
            ...
            # létrehozok egy label-t és a label-nek állítom be a képet mint háttért image
...
class DetailFrame(ttk.Frame):
    def __init__(self, container, controller, image_data):
        ...
        play_button = ttk.Button(
            self,
            text="Play ME",
            command=lambda: os.system("start " + r"C:\WORK\Prooktatás\projekt_ui\prooktatas_oop_project\movies\Prometheus.mkv")
            #command=lambda: os.system("start " + image_data['data_location'])
        )

        play_button.grid(row=2, column=1, sticky="W")

        def reconfigure_message_labels(envet):
